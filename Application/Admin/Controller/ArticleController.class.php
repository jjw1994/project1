...
class ArticleController extends AdminController
{
    ...
    public function index($cate_id = null, $model_id = null, $position = null, $group_id = null)
    {
        ...
        /*
         * @author 钟锦
         * 说明 添加了获取用户uid并传到getDocumentList()的代码
         */
        $uid = $_SESSION['onethink_admin']['user_auth']['uid'];
        $list = $this->getDocumentList($cate_id, $model_id, $position, $fields, $group_id, $uid);
        // 列表显示处理
        ...
        for($i = 0; $i < count($list); $i ++){
            $map_d['id'] = $list[$i]['id'];
            $map_u['uid'] = $document->where($map_d)->getField('uid');
            $username = $user->where($map_u)->getField('nickname');
            $list[$i]['type'] = $username;

        }
        for($j = 0; $j < count($grids); $j ++){
            if($grids[$j]['field'][0] == "type"){
                $grids[$j]['title'] = "作者";
            }
        }
     ...
     * 说明 在函数里添加$uid形参
     */
    protected function getDocumentList($cate_id = 0, $model_id = null, $position = null, $field = true, $group_id = null, $uid = null)
    {
        /* 查询条件初始化 */
        $map = array();
        if (isset($_GET['title'])) {
    ...
    public function add()
    {
        ...
        $map['category_id'] = $cate_id;
        $tag = $blog_article_tag->where($map)->field('id,tag_name')->order('sort')->select();
        $this->assign('tag', $tag);
    ...
    {
        $info = I('get.');
        $info['category_id'] = $info['cate_id'];
        $blog_article_tag = D('blog_article_tag');
        $map['category_id'] = $info['cate_id'];
        unset($info['cate_id']);
        $tag = $blog_article_tag->where($map)->field('id,tag_name')->order('sort')->select();
        $this->assign('info', $info);
        $this->assign('tag', $tag);
    ...
    public function mdedit()
    {
        ...
        $article = $Document->detail($info['id']);
        ...
        if(IS_POST){
              $post = I("post.");
              if($post["markdown"]){
                    $this->ajaxReturn($article['content']);
              }
        }
    ...
    /**
     * LSGOPHPFunctionInfo
     * function introduce:markdown编辑文章
     * @param null
     * @return null
     * @author Aidan Dai
     */
    public function mdupdate(){
        $post = I("post.");
        ...
        $bool = $document->where($map)->save($post) || $document_article->where($map)->save($detail);
