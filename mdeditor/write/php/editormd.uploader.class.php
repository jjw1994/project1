<?php
    ...
    */
    ...
    class EditorMdUploader
    {
        ...
        public $randomLength   = 0;            // 生成随机文件名的长度，当为日期时为日期的格式
        ...
        public function __construct($savePath, $saveURL, $formats, $randomNameType = 2, $randomLength = 10, $cover = true, $maxSize = 1024)
        {
        ...
        {
            $exp = pathinfo($this->files['name'], PATHINFO_EXTENSION);//获取后缀
            $fileName = "";
            if ($this->randomNameType == 1)        // 生成时间格式文件名
            {
                date_default_timezone_set('PRC');  //设置时区
                $date     = date($this->randomLength);
                $fileName = $date . "_" . mt_rand(10000, 99999).'.'.$exp;
            }
